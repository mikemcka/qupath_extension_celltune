...
- **Fidelity gaps vs stock scanpy** (documented, not defects — see USER_GUIDE.md §11.5): (1) CWTS Leiden optimises CPM, not scanpy's default modularity/RBConfiguration; (2) SNN/Jaccard edge weighting, not UMAP fuzzy-simplicial-set connectivities; (3) no PCA dimensionality reduction before the neighbour graph (near-identity for a ~20-marker panel). An external `sc.tl.leiden` run on the same data is not guaranteed to reproduce bit-identical cluster boundaries.
