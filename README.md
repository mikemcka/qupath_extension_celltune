...
5. **Normalise features** (optional) — *Extensions > CellTune Classifier > Normalise Features* — apply arcsinh or sqrt transforms to selected features. Use cofactor=1 for fluorescence (COMET, CODEX) or cofactor=0.05 for MIBI mass spectrometry (Hartmann et al. 2021; see [References](#references)).
...
- **Normalization methods (imaging)** — general background on tabular normalization approaches for highly multiplexed imaging (arcsinh, *background identification*, CLR, z-score, min-max, reference-channel) is catalogued in the Schapiro Lab's **[norm_methods](https://github.com/SchapiroLabor/norm_methods)** repository, which informs CellTune's feature-normalisation and cofactor guidance (§4.2 of the [User Guide](USER_GUIDE.md)).
